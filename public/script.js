...
function toast(msg, type = 'info', dur = 3500) {
  ...
  el.innerHTML = `<i class="ti ${icons[type] || icons.info}"></i><span class="toast-text">${msg}</span><button class="toast-close" type="button" aria-label="Скрыть"><i class="ti ti-x"></i></button>`;
...
function renderRequests() {
  ...
  if (sentFriendRequests.length) {
    ...
    sentFriendRequests.forEach(req => {
      ...
      li.appendChild(ava);
      li.innerHTML += `
        <div class="ci-body"><span class="ci-name">${req}</span><span class="ci-sub">Вы отправили заявку</span></div>
        <div class="req-btns">
          <button class="req-cancel" onclick="cancelSentReq('${req}')"><i class="ti ti-x"></i><span>Убрать</span></button>
        </div>`;
      ul.appendChild(li);
    });
  }
      ...
      resultsEl.innerHTML = searchResults.map(u => {
        ...
        const friendBadge = u.isFriend
          ...
          : u.hasOutgoingRequest
            ? `<button class="btn-secondary" style="padding:5px 10px;font-size:11px;flex-shrink:0" onclick="event.stopPropagation(); cancelSentReq('${u.username}')"><i class="ti ti-clock-x"></i> Убрать</button>`
            : u.hasIncomingRequest
              ? `<span style="font-size:10px;color:var(--accent);background:var(--accent-dim);border-radius:6px;padding:2px 7px;border:1px solid rgba(99,102,241,.22)"><i class="ti ti-arrow-down"></i> Входящая</span>`
              : `<button class="btn-secondary" style="padding:5px 10px;font-size:11px;flex-shrink:0" onclick="event.stopPropagation(); sendFriendReqTo('${u.username}')"><i class="ti ti-user-plus"></i> Добавить</button>`;
        return `
  ...
  try {
    ...
    if (d.success) {
      if (d.autoAccepted) {
        ...
        renderFriends();
        toast(`${username} теперь у вас в друзьях`, 'success');
      } else {
        sentFriendRequests = d.sentFriendRequests || [...new Set([...sentFriendRequests, username])];
        toast(`Заявка отправлена ${username}!`, 'success');
      }
      renderRequests();
    }
...
async function cancelSentReq(req) {
  try {
    ...
    if (d.success) {
      ...
      renderRequests();
      toast(`Заявка для ${req} убрана`, 'info', 2200);
    } else {
